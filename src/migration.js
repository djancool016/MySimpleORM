class Migration {
    static queryBuilder({columns, tableName, timestamp, db_system}){
        try {
            ...
            switch(db_system){
                case 'mysql':
                    query = mysqlMigrationQueryBuilder(tableName, columns, timestamp)
                    break
                case 'postgres':
                    query = postgresMigrationQueryBuilder(tableName, columns, timestamp)
                    break
                default:
    ...
    static async migrate({migrations, db, db_system = 'postgres'}){
        ...
        try {
            ...
            for(const key in migrations){
                ...
                const{tableName, columns, timestamp} = migrations[key]

                try {
                    if(!Array.isArray(columns) || columns.length < 1){
                        throw new Error('NoColumnsDefined')
                    }
                    const query = await Migration.queryBuilder({columns, tableName, timestamp, db_system})
                    await runQuery(db, query, tableName, columns, db_system)
...
async function runQuery(db, query, tableName, columns, db_system){
    switch(db_system){
        case 'mysql':
...
function postgresMigrationQueryBuilder(tableName, columns, timestamp){
    // create column string query
    const colQuery = columns.map( col => {

        let dataType = col.dataType
        let nullable = col.nullable ? '' : 'NOT NULL'
        let references = col.references ? `REFERENCES ${col.references.table}(${col.references.key}) DEFERRABLE INITIALLY DEFERRED` : ''
        const autoIncrement = col.autoIncrement ? 'SERIAL PRIMARY KEY' : ''
        ...
        // Auto Increment removing datatype and nullable
    ...
    const updatedAt = timestamp ? `updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP` : ''

    let isTimeStamp = timestamp? `, ${createdAt} , ${updatedAt}` : ''

    return `${colQuery} ${isTimeStamp}`.replace(/\s+/g, ' ')
}

function mysqlMigrationQueryBuilder(tableName, columns, timestamp){
